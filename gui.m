...
% Last Modified by GUIDE v2.5 28-May-2022 01:00:42
...
% --- Executes on button press in input_citra.
function input_citra_Callback(hObject, eventdata, handles)
% hObject    handle to input_citra (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
output = uint8(zeros(size(rgb,1), size(rgb,2), size(rgb,3)));
...
red=mean(mean(output(:,:,1)));
green=mean(mean(output(:,:,2)));
blue=mean(mean(output(:,:,3)));
